	...
	while (current_menu != EXIT_PROGRAM) {
		...
		case RESERVATION_MANAGEMENT_MENU:
			switch (read_option_and_show_menu(show_reservation_management_menu)) {
				...
				case '2':
					display_reservations(&ls_reservations);
					pause_and_continue();
					break;
		...
		case ADD_ROOM_MENU:
			show_add_room_menu();
			if (add_hall_parsing(&ls_halls) == INPUT_GO_BACK) { current_menu = ROOM_MANAGEMENT_MENU; save_data_halls(); }
			break;

		case DELETE_ROOM_MENU:
			show_delete_room_menu();
		    if (remove_hall_parsing(&ls_halls, &ls_reservations) == INPUT_GO_BACK) { current_menu = ROOM_MANAGEMENT_MENU; save_data_halls(); save_data_reservations(); }
			break;
		...
		case SEARCH_ROOM_BY_NAME_MENU:
			...
			if (search_hall_by_name_parsing(&ls_halls) == INPUT_GO_BACK) { current_menu = SEARCH_ROOM_MENU; } 
			break;

		case SEARCH_ROOM_BY_CAPACITY_MENU:
			show_search_room_by_capacity_menu();
			if (search_hall_by_capacity_parsing(&ls_halls) == INPUT_GO_BACK) { current_menu = SEARCH_ROOM_MENU; }
			break;

		case SEARCH_ROOM_BY_AVAILABILITY_MENU:
			show_search_room_by_availability_menu();
			if (search_hall_by_availability_parsing(&ls_halls) == INPUT_GO_BACK) { current_menu = SEARCH_ROOM_MENU; }
			break;

		case RESERVE_ROOM_MENU:
			show_reserve_room_menu();
			if (add_reservation_parsing(&ls_reservations, &ls_halls) == INPUT_GO_BACK) { current_menu = RESERVATION_MANAGEMENT_MENU; save_data_reservations(); }
			break;

		case CANCEL_RESERVATION_MENU:
			show_cancel_reservation_menu();
			if (cancel_reservation_parsing(&ls_reservations, &ls_halls) == INPUT_GO_BACK) { current_menu = RESERVATION_MANAGEMENT_MENU; save_data_reservations(); save_data_halls(); }
			break;
